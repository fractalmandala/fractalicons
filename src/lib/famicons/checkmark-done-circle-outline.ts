// Generated by scripts/generate-icons.js. Do not edit by hand.
import type { IconData } from '../types.js';

const faCheckmarkDoneCircleOutline = {
	"name": "checkmark-done-circle-outline",
	"set": "famicons",
	"viewBox": "0 0 512 512",
	"body": "<path d=\"M448,256c0-106-86-192-192-192S64,150,64,256s86,192,192,192S448,362,448,256Z\" style=\"fill:none;stroke:currentColor;stroke-miterlimit:10;stroke-width:32px\"/><polyline points=\"368 192 256.13 320 208.18 272\" style=\"fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px\"/><line x1=\"191.95\" y1=\"320\" x2=\"144\" y2=\"272\" style=\"fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px\"/><line x1=\"305.71\" y1=\"192\" x2=\"254.16\" y2=\"251\" style=\"fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px\"/>"
} as const satisfies IconData;

export default faCheckmarkDoneCircleOutline;
export { faCheckmarkDoneCircleOutline };
export { faCheckmarkDoneCircleOutline as famiconsCheckmarkDoneCircleOutline };
