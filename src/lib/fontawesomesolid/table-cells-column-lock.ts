// Generated by scripts/generate-icons.js. Do not edit by hand.
import type { IconData } from '../types.js';

const fasTableCellsColumnLock = {
	"name": "table-cells-column-lock",
	"set": "fontawesomesolid",
	"viewBox": "0 0 640 640",
	"body": "<path fill=\"currentColor\" d=\"M256 288L256 480L320 480L320 288L256 288zM512 209C465.3 214.8 426.4 245.8 409.3 288L384 288L384 378.8C373.9 394 368 412.2 368 431.9L368 527.9C368 533.4 368.5 538.8 369.3 544L128 544C92.7 544 64 515.3 64 480L64 160C64 124.7 92.7 96 128 96L448 96C483.3 96 512 124.7 512 160L512 209zM128 288L128 480L192 480L192 288L128 288zM560 336.1C560 318.4 545.7 304.1 528 304.1C510.3 304.1 496 318.4 496 336.1L496 384L560 384L560 336.1zM416 432C416 411.1 429.4 393.3 448 386.7L448 336.1C448 291.9 483.8 256.1 528 256.1C572.2 256.1 608 291.9 608 336.1L608 386.7C626.6 393.3 640 411.1 640 432L640 528C640 554.5 618.5 576 592 576L464 576C437.5 576 416 554.5 416 528L416 432z\"/>"
} as const satisfies IconData;

export default fasTableCellsColumnLock;
export { fasTableCellsColumnLock };
export { fasTableCellsColumnLock as fontawesomesolidTableCellsColumnLock };
