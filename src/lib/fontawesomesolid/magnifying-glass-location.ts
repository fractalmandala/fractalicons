// Generated by scripts/generate-icons.js. Do not edit by hand.
import type { IconData } from '../types.js';

const fasMagnifyingGlassLocation = {
	"name": "magnifying-glass-location",
	"set": "fontawesomesolid",
	"viewBox": "0 0 640 640",
	"body": "<path fill=\"currentColor\" d=\"M480 272C480 317.9 465.1 360.3 440 394.7L566.6 521.4C579.1 533.9 579.1 554.2 566.6 566.7C554.1 579.2 533.8 579.2 521.3 566.7L394.7 440C360.3 465.1 317.9 480 272 480C157.1 480 64 386.9 64 272C64 157.1 157.1 64 272 64C386.9 64 480 157.1 480 272zM352 240C352 195.8 316.2 160 272 160C227.8 160 192 195.8 192 240C192 288.8 238.5 351.6 260.6 378.6C266.6 385.9 277.4 385.9 283.3 378.6C305.4 351.6 351.9 288.8 351.9 240zM240 240C240 222.3 254.3 208 272 208C289.7 208 304 222.3 304 240C304 257.7 289.7 272 272 272C254.3 272 240 257.7 240 240z\"/>"
} as const satisfies IconData;

export default fasMagnifyingGlassLocation;
export { fasMagnifyingGlassLocation };
export { fasMagnifyingGlassLocation as fontawesomesolidMagnifyingGlassLocation };
